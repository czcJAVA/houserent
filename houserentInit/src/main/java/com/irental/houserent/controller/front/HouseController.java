...
import com.irental.houserent.common.util.ClickBasedCFRecommender;
...
public class HouseController extends BaseController {
    ...
    public String houseList(HouseSearchVO houseSearchVO, Model model) {
        ...
        // 1. 添加用户点击数据 (userId, itemId)
        List<Long[]> clickRecords = Arrays.asList(
                new Long[]{1L, 101L},
                new Long[]{1L, 102L},
                new Long[]{1L, 101L}, // 用户1多次点击101
                new Long[]{2L, 101L},
                new Long[]{2L, 103L}
        );
        ClickBasedCFRecommender.addClickRecords(clickRecords);
        List<Long> recommendations = ClickBasedCFRecommender.recommendItems(1L, 3);

        ClickBasedCFRecommender.addClickRecord(1L, 103L);
        recommendations = ClickBasedCFRecommender.recommendItems(1L, 3);
        System.out.println("更新后的推荐: " + recommendations);
